...
const TDP_ROLES = ['superadmin', 'admin', 'director', 'project_manager', 'developer', 'sales', 'support', 'finance', 'staff', 'client'];
...
export function Users() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const json = await api('/api/tdp/users');
      if (json.ok) setUsers(json.users || []);
      else setError(json.error || 'Error loading users');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const json = await api('/api/tdp/users', { method: 'POST', body: JSON.stringify(form) });
    if (json.ok) {
      setUsers(prev => [...prev, json.user]);
      setShowCreate(false);
      setForm({ email: '', full_name: '', role: 'staff', password: '' });
    } else setError(json.error || 'Error creating user');
  };

  const handleToggleActive = async (user: any) => {
    const newStatus = user.status === 'active' ? 'inactive' : 'active';
    const json = await api(`/api/tdp/users/${user.id}`, { method: 'PATCH', body: JSON.stringify({ status: newStatus }) });
    if (json.ok) setUsers(prev => prev.map(u => u.id === user.id ? json.user : u));
  };

  const handleRoleChange = async (user: any, role: string) => {
    const json = await api(`/api/tdp/users/${user.id}`, { method: 'PATCH', body: JSON.stringify({ role }) });
    if (json.ok) setUsers(prev => prev.map(u => u.id === user.id ? json.user : u));
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gestión de usuarios del sistema TDP Admin</p>
        </div>
      ...
      {error && <div className="bg-red-50 border border-red-200 text-red-600 p-3.5 rounded-xl text-sm">{error}</div>}

      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-100">
              <h3 className="text-lg font-bold text-gray-800">Nuevo usuario TDP</h3>
            ...
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              <div>
                ...
                <input type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                  className="w-full border border-gray-200 rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-[#009FE3]" placeholder="usuario@tallerdepixeles.com" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-700 mb-1 block">Nombre completo</label>
                <input type="text" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))}
                  className="w-full border border-gray-200 rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-[#009FE3]" placeholder="Nombre del usuario" />
              </div>
              <div>
                ...
                <select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                  ...
                  {TDP_ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-700 mb-1 block">Contraseña</label>
                <input type="password" required value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
                  className="w-full border border-gray-200 rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-[#009FE3]" placeholder="••••••••" />
              </div>
              <button type="submit" className="w-full bg-[#009FE3] text-white font-semibold py-2.5 rounded-xl hover:bg-[#0088c4] text-sm">
                Crear usuario
              </button>
            </form>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        : users.length === 0 ? (
          <div className="p-12 text-center"><UsersIcon className="mx-auto text-gray-300 mb-3" size={48} /><p className="text-gray-500">No hay usuarios registrados</p></div>
        ) : (
            ...
            <tbody>
              {users.map(user => (
                <tr key={user.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${user.status === 'active' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleToggleActive(user)}
                      className={`p-2 rounded-lg transition-all ${user.status === 'active' ? 'hover:bg-red-50 text-red-400' : 'hover:bg-green-50 text-green-500'}`}
